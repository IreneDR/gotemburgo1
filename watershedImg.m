...
IB=imread(uigetimagefile());
IB=double(IB)/65536; 
insideWell = bwmorph(imfill(imclose(imbinarize(IB), strel('disk', 1)), 'holes'), 'majority');
IB(imerode(bwareafilt(insideWell, 1), strel('disk', 20)) == 0) = 0;

%% why do we get to IB>180???
IB_filt=imgaussfilt(IB,6); 
IB=IB-IB_filt;
IB=imcomplement(IB);
%Normalize 0-255
IB=((IB-min(IB(:)))./(max(IB(:))-min(IB(:)))).*255;
outline = bwareafilt(IB>180, 1);
IB(outline) = 0;
closeimg= imclose (IB>180, strel('disk',2));
fillimg= imfill (closeimg, 'holes');
suma= double(IB>180) + double(fillimg);
figure, imshow(suma+1, parula(3))
figure, imshow(suma == 1)
%Trying to remove the outer objects that are not cells
backgrounddil= imdilate (suma == 0,  strel('disk', 1)); 
figure, imshow (backgrounddil & suma==1, parula (3))
labeledsuma= bwlabel (suma==1, 4)
biggestObject = bwareafilt(suma==1, 1);
figure, imshow(biggestObject)
smallBW2= bwareafilt(biggestObject==0,[1 7], 4);
figure, imshow (smallBW2)

figure, imshow (labeledsuma+1, parula (20))
sumafinal= (biggestObject + smallBW2)

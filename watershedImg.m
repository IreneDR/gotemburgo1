...
%     frameFiles = dir('data\output\segmentedcells\Image #*');
    ...
    labelledImg1 = bwlabel(resizedImg1 & OverlappingImg>0);
    labelledImg2 = bwlabel(resizedImg2 & OverlappingImg>0);
    ...
    for numCell= 1:max (max (labelledImg1))
        ...
        trackingCells(numCell, 2) = {unique(labelledImg2(labelledImg1 == numCell))};
    end
...
%     % Visualize the corresponding points. You can see the effect of translation between the two images despite several erroneous matches.
    ...
    figure('units','normalized','outerposition',[0 0 1 1], 'visible', 'on' ); showMatchedFeatures(I1,I2,matchedPoints1,matchedPoints2);
